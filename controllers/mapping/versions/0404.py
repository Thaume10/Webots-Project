...
def movetotarget(objectif,wheels,ps, robot,discovery_map):
    pose=fetchgps(gps,gps_value)
    ...
    while (abs(objectif[0]-pose[0])>0.05)or(abs(objectif[1]-pose[1])>0.05) :
        ...
        right_obstacle =  psValues[0] < 1000.0 or psValues[1] < 1000.0  or psValues[9] < 500.0
        left_obstacle = psValues[6] < 1000.0 or psValues[7] < 1000.0 or psValues[8] < 500.0
        ...
        if left_obstacle and (close==False):
            ...
            print("turn left")
            move(-MAX_SPEED_ANGULAR,MAX_SPEED_ANGULAR,wheels)
        elif ((arounf_left and delta>0  ) or (arounf_right and delta<0)) and (close==False):
             print("contourne")
             move(vL, vR, wheels)
        else:
            print("go cible")
...
def getOG(lidar_front,lidar_angles_front, lidar_back, lidar_angles_back,pose,occupancy_map): #code aufräumen und funktionen integrieren mit dem Skalieren wie im loop gemacht. Bzw kann man doppelte werte rauslöschen?
    ...
    for i in range(len(lidar_distances_front)):
        ...
        if lidar_distances_front[i]==np.inf:
                empty_lidar_distances_front.append(2)
    ...
    for i in range(len(lidar_distances_back)):
        ...
        if lidar_distances_back[i]==np.inf:
                empty_lidar_distances_back.append(2)
...
def saveOGasCSV(occupancy_map):
        #Save Mapdata
        occupancy_map=occupancy_map.astype(int)
        np.savetxt("Occupancy_Map.csv", occupancy_map, fmt='%i', delimiter=",")
        
        # Plot the occupancy grid map
        plt.imshow(occupancy_map, cmap='Greys', extent=[0, MapWidth, 0, MapHeight]) #Problem with y data. it plots the data ine away from the border
        plt.xlabel('X (meters)')
        plt.ylabel('Y (meters)')
...
def MappingPoint(discovery_map):
        ...
        print(discovery_map, 'look')
        #Searches for field in discovery Map wich is 0
        found = False
        for y in range(len(discovery_map)):
                for x in range(len(discovery_map[y])):

                        if discovery_map[y][x]==1:
                                pass

                        elif discovery_map[y][x]==0:
                                MappingPointX=x+1
                                MappingPointY=MapHeight-y
                                found=True
                                break
                if found == True:
                       break
...
if __name__ == "__main__":
    ...
    while robot.step(TIME_STEP) != -1:
        ...
        if a==20:
            ...
            #print(MappingData)
            initDisplay(robot, MappingData)
            discoverymap(pose)
            print(discovery_map)
            print(100*((discovery_map.sum())/(MapHeight*MapWidth)),'%')
            if (100*(discovery_map.sum())/((MapHeight)*(MapWidth))) <= 99 :
                [MappingPointX, MappingPointY] = MappingPoint(discovery_map)
            movetotarget([MappingPointX, MappingPointY],wheels,psValues,robot,discovery_map)
