...
def getOG(lidar_front,lidar_angles_front, lidar_back, lidar_angles_back,pose,occupancy_map): #code aufräumen und funktionen integrieren mit dem Skalieren wie im loop gemacht. Bzw kann man doppelte werte rauslöschen?
    ...
    for x in range(len(filtered_lidar_angles_front)):
        #Front readings
        MappingData[0].append(((pose[0]+0.55*np.cos(angle)+filtered_lidar_distances_front[x]*np.cos(filtered_lidar_angles_front[x]+angle)))) #shows only the values that are read in the moment #the pose of the robot is included in this vector. Why?
        MappingData[1].append((pose[1]+0.55*np.sin(angle)+filtered_lidar_distances_front[x]*np.sin(filtered_lidar_angles_front[x]+angle))) #shows only the values that are read in the moment #the pose of the robot is included in this vector. Why?
    ...
    for y in range(len(filtered_lidar_angles_back)):
        #Back readings
        MappingData[2].append(((pose[0]-0.55*np.cos(angle)-filtered_lidar_distances_back[y]*np.cos(filtered_lidar_angles_back[y]+angle)))) #shows only the values that are read in the moment #the pose of the robot is included in this vector. Why?
        MappingData[3].append((pose[1]+0.55*np.sin(angle)+filtered_lidar_distances_back[y]*np.sin(filtered_lidar_angles_back[y]+angle))) #shows only the values that are read in the moment #the pose of the robot is included in this vector. Why?
    ...
    for i in range(len(MappingData[0])):
        ...
        map_x = int(math.floor(MappingData[0][i]/cellSize))
        map_y = int(math.floor((MapHeight/cellSize)-(MappingData[1][i]/cellSize)))
