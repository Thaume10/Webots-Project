...
def improve_grid(grid):
    for i in range(len(grid)):
        for j in range(len(grid[0])):
            ...
            if grid[i][j] == '0' and nb_neighbors < 2:
                grid[i][j] = '1'
            elif grid[i][j] == '1' and nb_neighbors <2 :
                grid[i][j] = '0'
            elif grid[i][j] == 1 and nb_neighbors <2 :
                grid[i][j] = 0
            elif grid[i][j] == 0 and nb_neighbors <2 :
                grid[i][j] = 1
            else:
                grid[i][j] = grid[i][j]
    #
...
def mapping(wheels,ps, robot,occupancy_map):
    ...
    while calculate_percentage_not_equal_to_minus_one(occupancy_map) <100 :
        ...
        if a==20:
            ...
            if(occupancy_map[Y][X]!=-1):
                objectif[0],objectif[1],X,Y=NewTarget(occupancy_map)
            print("objectif :", objectif)
        a=a-1
        #objectif=NewTarget(occupancy_map)
        robot.step(TIME_STEP)
        psValues = []
        ...
        if psValues[10] < 1000.0 and (close==False):
            #print("turn iii")
            for i in range(17):
                robot.step(TIME_STEP)
    ...
    move(vL, vR, wheels)
    print('Mapping finsihed, the file is in the controller folder')
    occupancy_map = np.flip(occupancy_map, axis=0)
    occupancy_map=improve_grid(occupancy_map)
    exportOccupancyGridAsCsvSafely(occupancy_map)

#############################################################################################

def bresenham_with_threshold(x0, y0, x1, y1, threshold=0.3):
    dx = abs(x1 - x0)
    dy = abs(y1 - y0)
...
def getOG(lidar_front,lidar_angles_front, lidar_back, lidar_angles_back,pose,occupancy_map): 
    #Get reading values from front lidar
    distances_fr=lidar_front.getRangeImage()
    ...
    for i in range(len(distances_fr)):

        if distances_fr[i]!= np.inf:
                filtered_dist_fr.append(distances_fr[i])
                filtered_angles_fr.append(lidar_angles_front[i])
    
    #Get reading values from back lidar
    dist_bk=lidar_back.getRangeImage()
    ...
    for i in range(len(dist_bk)):
        if dist_bk[i]!= np.inf:
                filtered_dist_bk.append(dist_bk[i])
                filtered_angles_bk.append(lidar_angles_back[i])
...
#process information
    ...
    for x in range(len(filtered_angles_fr)):
        #Front readings
        Data[0].append(((pose[0]+0.45*np.cos(angle)+filtered_dist_fr[x]*np.cos(filtered_angles_fr[x]+angle))))
        Data[1].append((pose[1]+0.45*np.sin(angle)+filtered_dist_fr[x]*np.sin(filtered_angles_fr[x]+angle)))

    angle= -pose[2]*np.pi/(180)

    for y in range(len(filtered_angles_bk)):
        #Back readings
        Data[2].append(((pose[0]-0.45*np.cos(angle)-filtered_dist_bk[y]*np.cos(filtered_angles_bk[y]+angle))))
        Data[3].append((pose[1]+0.45*np.sin(angle)+filtered_dist_bk[y]*np.sin(filtered_angles_bk[y]+angle)))
    ...
    center_x = int(round(pose[0] * 10))
    center_y = int(round(len(occupancy_map) - pose[1] * 10))
    for i in range(len(Data[0])):

        # Convert robot coordinates to map coordinates (assuming robot at origin)
        import math

        map_x = int(math.floor(Data[0][i]/cellSize))
        map_y = int(math.floor((MapHeight/cellSize)-(Data[1][i]/cellSize)))
...
def NewTarget(occupancy_map):
    closest_distance = 10000
    NewTargetX, NewTargetY,xfin,yfin = 0, 0,0,0
    pose=fetchgps(gps,gps_value)
        ...
        for x in range(len(occupancy_map[y])):
            ...
            if  occupancy_map[y][x] == -1 :
                ...
                if distance < closest_distance:
                    ...
                    NewTargetX = (x+1)*0.1 
                    NewTargetY = len(occupancy_map)*cellSize - y*0.1
                    xfin=x
                    yfin=y
    
    return NewTargetX, NewTargetY,xfin,yfin
